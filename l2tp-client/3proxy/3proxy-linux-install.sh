...
ReserveDirBin=/tmp-proxy/bin
ReserveDirLib=/tmp-proxy/lib
ReserveDirMan3=/tmp-proxy/man3
ReserveDirMan8=/tmp-proxy/man8
ReserveDirInit=/tmp-proxy/init.d
if [ ! -d "$ReserveDirBin" ]; then mkdir $ReserveDirBin; fi
if [ ! -d "$ReserveDirLib" ]; then mkdir $ReserveDirLib; fi
if [ ! -d "$ReserveDirMan3" ]; then mkdir $ReserveDirMan3; fi
if [ ! -d "$ReserveDirMan8" ]; then mkdir $ReserveDirMan8; fi
if [ ! -d "$ReserveDirInit" ]; then mkdir $ReserveDirInit; fi
...
SetInit()
{
	...
	cp -vf /etc/init.d/3proxy $ReserveDirInit
	PacketFiles=`echo -e "${PacketFiles}\n/etc/init.d/3proxy"`
